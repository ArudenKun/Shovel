...
namespace Shovel.Extensions.Markup;

public class Paths : MarkupExtension
{
    public DependencyProperty From { get; set; } = null!;

    public string To { get; set; } = null!;

    public override object ProvideValue(IServiceProvider serviceProvider)
    {
        var provideValueTarget = (IProvideValueTarget)
            serviceProvider.GetService(typeof(IProvideValueTarget))!;
        var targetObject = (UIElement)provideValueTarget.TargetObject;
        return new OneWayToSource.ProxyBinding(targetObject, From, To);
    }
}
...
public static class OneWayToSource
{
    ...
    public class ProxyBinding : DependencyObject, IDisposable
    {
        ...
        private static void OnSourceProxyChanged(
            DependencyObject d,
